...
## [2026.9.22] - 2026-09-20

- The room page's "Rarely needed" fold is gone. Its sun entity setting moves to the house page, next to the outdoor light-level sensor, as a house-wide `Global.SunEntity` — it no longer takes a per-room override, since a house has exactly one. Its fade-time settings (fade when light out, fade when dark out) move into the room's darkness fold, renamed "Darkness and fading" to match. A document that still names `SunEntity` under a room or under `Defaults` now loads with a warning saying the key does nothing, in the log and on the page, and the next save drops it; the house reads `Global.SunEntity`, `sun.sun` by default. The house's own `Global.SunEntity` is validated like the outdoor light-level sensor: an id in the wrong domain, or one Home Assistant does not know, is warned about rather than left to fail quietly at dusk.
...
- Lamplight's room header says the room's state once. The light switch keeps its two buttons and loses the "Lights ON/OFF" label beside them, since the card's own large reading above already says it. The adaptive lighting switch leaves the header for the "This room" section; in its place, a room that is switched off gets one "Activate adaptive lighting" button, which now also makes the room run immediately. The large reading, the "right now" facts and the line beneath them are unchanged.
- A setting card in Lamplight's room and house settings grids now shows what it controls before what it is set to. The name used to sit small under a large, bold current value, answering "what is the value" before "what is being controlled"; the name now reads first, in the stronger weight, with the value a smaller, coloured line underneath it. One shared component (`Row`), so every screen using it — a room's own settings and the house-wide defaults — changed together.
...
- A level test running when a house-mode scene fires no longer ends by aiming the room off that scene. The test's return re-runs the scene the room is carrying, as every other re-aim already does.
- A room's on/off switch on the house page is more reliable in three ways. A write the file refuses now puts the switch back where the file left it, instead of showing the room as switched while nothing was written. Switching a whole floor and then a single room on that floor no longer reports the room as "changed somewhere else": the page's own unsaved edits are no longer mistaken for somebody else's write. And a press that reached the file is reported as saved even if the page cannot re-read the document straight afterwards.
- The room page no longer credits a scene, or the engine, for lights a person set by hand. A house-mode scene firing after a manual override used to add a "Scene" row and hand "Last changed" to the engine, because the scene's timestamp was newer; the person's own change now wins in any state the engine is not aiming.
...
- Lamplight's six themes now show their own faces: 19 unmodified, Latin-subset `.woff2` files under `AdaptiveLighting.Lamplight/wwwroot/fonts/`, one folder per family with its SIL Open Font License 1.1 text beside it, served by the app with nothing fetched at run time. A browser without a file falls through the theme's stack to a system font.
- Short text on screen in both designs. Notices, empty states, headlines, next-change lines, fold summaries and activity rows are a few words each; the explanation that used to fill the line sits behind an (i). The (i) in both designs takes a severity, `InfoSeverity`: neutral for plain help, yellow for worth knowing, red for needs attention, coloured from each theme's `--warn` and `--bad`. `ActivityLine` gains `More`, the explanation drawn behind the row's (i); `ActivityView.HiddenWhy`, `ActivityPageModel.FootWhy`, `DashboardPageModel.MasterNoteMore`/`MasterNoteSeverity` and `RoomPageModel.DaylightSourceWhy` carry the rest. Validator messages show setting names as words (`Exclude label`) rather than the settings file's keys (`Global.ExcludeLabel`).
- Lamplight's port, its six themes and its bundled typefaces are documented in the settings reference, the user guide and the overview.
...
- **The activity log says who changed a light.** A manual change reads "By automation: *its name*", "By *person*" for somebody using Home Assistant, or "At the device or wall switch", the way Home Assistant's own logbook words it. A change an automation made that the room leaves alone now has a row of its own. The room page's *Last changed* names who made the newest change, "By adaptive lighting" included. Where no name can be found the row reads as before. A light that reports more than five seconds after an automation's command reads as the device, because Home Assistant no longer ties that report to the automation. Room reports carry the two new fields `changed_by` and `changed_at`.
...
- **A level of 0 % now switches the lights off, and the room carries on as normal.** A room, a period, the warning dim, the daylight curve's dark end or a sleep ceiling working out to nothing was sent as *switch on at nothing*, which Home Assistant carries out by switching the light off — and the room then believed somebody had done it by hand. Movement only restarted the ten-minute clock, and the next period's level did not arrive until the room had been quiet for it. Anything that works out to off is now sent as a real switch-off: the room stays in automatic, the lights come back by themselves at the next period, and Home Assistant's log shows a turn-off instead of a turn-on at zero. A group whose every lamp is set to nothing is no longer misread the same way, and pressing *Test* on a 0 % period switches the room off for the ten seconds and puts it back afterwards. A room already holding a 0 % somewhere will behave differently from now on: where it used to stall, it goes dark and keeps following the schedule.
...
- **Any single light in a room can now hold its own brightness, warmth or daylight-curve rule for a period, and the room keeps commanding everything else as before.** A reading lamp sits at 30 % while the rest of the room runs at 70 %; a ceiling light is off at night while the rest stays on. A level belongs to one light and never to a group, so a bulb reached through two groups holds one value and shows it under both, and a group of groups is the lights at the bottom of it. For one light in one period the answer is its own row where it states the value, then the room's, then the schedule; brightness and the curve flag travel together, so a lamp pinned to a number is not silently overruled by a curve the room follows, and warmth travels alone. The room page grows one line at the foot of its levels table, *Adjust lights individually*, offered only where the room reaches more than one light: it opens the engine's own settled list, a lamp as one line and a group as one line saying how many lights are under it and how many are set differently, each opening the same period table for that light alone. A light the room no longer reaches keeps what it was set to and is listed with a *Remove it* button. Nothing is stored for the checkbox itself — the rows are the fact — and unticking it says how many lights it would forget first. The ten-second period test resolves per light too, so it shows the room as it will really be lit. **A document naming no light levels reaches Home Assistant with the identical service calls, in the identical order, through movement, a boundary, the warning dim and the off**, which is what lets this reach a house with people in the rooms: nothing moves until somebody ticks the box in one. Where lights under a group are commanded individually the group entity still receives the detector's expectation and no command, or its re-publication of a member's change reads as a hand at a switch. A light set to nothing goes out as an off rather than a turn-on at nothing, which Home Assistant carries out as a turn-off against an on-expectation. The new `LightLevels` key on an area is additive and omitted from the file when empty, so an older build reads it as silence and commands every light at the room's level, exactly as it did before.
...
- **Brightness is stored as the 0-255 value Home Assistant accepts, so the fine-adjust handle can save what it sets.** The document holds `Brightness` where it held `BrightnessPct`; a whole percent covers two or three raw steps, so six steps up from 40 % used to show `( 108 / 255 )` on the handle, write `42` to the file and re-arm from 107. Everything a person reads at a glance is still a whole percent — the room's level row, the schedule, the sentences — and the handle is the one surface that names the byte, now taken from the stored value rather than from the rounded readout. The daylight curve endpoints and the pre-off dim factor are unchanged and stay in percent.

  **A document written in percent keeps commanding exactly what it commanded.** The old key is still bound on load and is simply never written again, so nothing converts underneath a running house; the byte grid is reached at the next ordinary save. Percent and raw are not a lossless pair — 74 of the 101 whole percents come back as a different number after a trip through a byte — so what the save holds is the part that matters, over every whole percent: the byte the lamp lands on, and the whole percent every readout shows. A hand-edited half percent is the one shape that reads differently afterwards: 62.5 % lands on 159 and reads as 62 rather than 63, which is 0.15 of a percentage point of light. A document stating both keys can only come from a hand edit; the raw one wins, the percentage is dropped with a warning, and the file is written back without it.
