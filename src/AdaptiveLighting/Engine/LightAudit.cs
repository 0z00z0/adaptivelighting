...
/// <summary>One light as the audit reads it.</summary>
// Name is the friendly_name, or the entity id when it has none.
public sealed record LightUnderReview(string EntityId, string Name);

/// <summary>One light the audit is not convinced is a light, and why, in a person's words.</summary>
// Reason is never a rule id: the reader has to be able to judge the guess.
public sealed record SuspectLight(string EntityId, string Name, string Reason);

/// <summary>One room as the cross-room half of the audit reads it.</summary>
// Lights holds bulbs with groups already followed down to their members, because two rooms reaching one bulb
// through two different groups settle on two different ids and have nothing in common to compare.
public sealed record RoomUnderReview(string Room, IReadOnlyList<LightUnderReview> Lights);

/// <summary>Looks over the lights a room would command and points at the ones that are probably not room lighting.</summary>
// Advice, never a filter. Home Assistant's entity_category, the field that would settle it, is not exposed by
// HassModel 26.21.0, so everything here is a heuristic on an id and a name. The rules are asymmetric: a word
// that accuses must match a whole word, while a word that excuses need only appear inside one, because Norwegian
// writes its lamps as taklys and benkbelysning.
public static class LightAudit
{
	...
	/// <summary>The lights in <paramref name="lights"/> that look like something other than room lighting, in the order given.</summary>
	...
	// present is every entity id in the same room, for the colour-channel sibling check. The order of the rules is
	// load-bearing: the status rule runs before the lamp guard, so a ceiling light's status LED is caught by the
	// word describing the entity before the word describing its device can excuse it.
	/// <summary>Why <paramref name="light"/> looks wrong, or <c>null</c> when it looks like a light.</summary>
	...
	// AreaEntityResolver drops a group reaching into another area and settles overlapping groups within one area;
	// neither catches a bulb belonging to no area, so both rooms resolve it and both command it. One entry per
	// bulb, never one per room. hasOwnArea is asked only about bulbs that already failed the cheap test, because
	// answering it sweeps the registry.
	/// <summary>The bulbs more than one room will command, because Home Assistant has not put them in a room at all.</summary>
	...
	// The entity this one is a colour channel of, when that entity is in the room too.
