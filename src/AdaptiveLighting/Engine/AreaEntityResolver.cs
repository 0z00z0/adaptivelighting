...
// Lets the configuration page ask what an area would resolve to through the engine's own rules, so no second
// approximation can drift from them. LuxSensors is the candidate list, never a chosen sensor.
public sealed record AreaDiscovery(
	IReadOnlyList<string> Lights,
	IReadOnlyList<string> MotionSensors,
	IReadOnlyList<string> LuxSensors);

/// <summary>Turns an <see cref="AreaConfig"/> into a <see cref="ResolvedArea"/>, discovering what the config left out.</summary>
// Discovery-first: an area id and a device class stay true across a rename, where a hand-listed entity does not.
public sealed class AreaEntityResolver
{
	...
	// Device classes and group membership come from ha; the registry answers areas, labels and devices.
	...
	/// <summary>What <paramref name="areaId"/> yields on its own, by the same rules <see cref="TryResolve"/> uses.</summary>
	// The exclude label, the group de-duplication and IsLive all apply. An area the registry does not know yields
	// nothing, and so does one whose own registration carries the exclude label, which keeps auto-discovery from
	// proposing it.
	...
	/// <summary>What one domain's groups are called, and what keeping a group alongside its own members costs there.</summary>
	// The selection is identical across the three domains; only the wording of the harm differs.
	// OneEntityPerDevice says whether several entities of this kind on one device are the same thing.
		...
		bool OneEntityPerDevice)
	{
		...
		// The device rule is off here. For a light a device is one lamp, while for motion it is a controller, and a
		// multi-zone presence sensor exposes genuinely different detection zones on one. Collapsing those blinds
		// the room wherever the dropped entity was watching, silently.
		...
		// The device rule is on because the area averages its sensors, and one instrument exposed as two entities
		// would carry double weight in that mean.
	...
	/// <summary>Settles an area's groups against each other and against their own members, in whichever domain.</summary>
	// A group and its members are the same entities twice; the members lose and the group wins. Three shapes
	// defeat a one-level comparison. Nesting: a group of groups lists groups, so membership is followed all the
	// way down through LeavesOf. Reach: a group may hold entities another room owns, which
	// WithoutGroupsReachingIntoAnotherArea decides. Overlap: two groups can share members without either
	// containing the other, so the widest coverage wins and the narrower group is traded for the members only it
	// holds. A bulb dropped from a room is worse than a bulb commanded twice. qualifies is the domain test a
	// promoted member must still pass.
	...
	/// <summary>Keeps one entity per piece of hardware: several entities on one Home Assistant device are one thing.</summary>
	// A device id is a registry fact where a name suffix is a guess, so where both could answer the device id is
	// believed. A group helper has no device of its own, so it is never a duplicate; it stands for the devices
	// beneath it and, having claimed them, leaves nothing for the loose entities on the same hardware. For a
	// device no group covers the order is: group, then breadth, then the entity its siblings extend with an
	// underscore, then the shortest id ordinally. The last two keep the answer independent of registry order. A
	// candidate covering several devices of which only some are claimed is kept, or the hardware nobody else
	// holds would be lost.
	...
	// The candidates whose siblings on the same device are named as extensions of them: the combined entity of a
	// multi-channel fixture. The underscore is load-bearing, or light.stue_tak_1 counts as the parent of
	// light.stue_tak_11, a different lamp on a different device.
	...
	// Every device an entity stands for: its own, or those of everything it groups. A group has no device of its
	// own, so without following the membership it would stand for and claim nothing.
	...
	// commanding what is inside it, so the area boundary wins and the room falls back to what it owns. Assigned
	// to no area at all does not count as foreign: an entity whose group carries the area assignment is the
	// ordinary way to set a house up, and treating it as another room's would clip nearly every group. An area
	// left with nothing after the clip keeps its reaching group, and the warning is the only signal.
	...
	// A reverse sweep, because IAreaRegistry answers what is in an area and the question is the other way round.
	// An entity this area holds itself is never foreign, whatever another area claims.
	...
	// selection pass settles every combination without special cases. A group may contain itself, so the walk
	// visits each id once; a loop bottoms out with no leaves, and such a group stands for itself and everything
	// it reaches. Public because the cross-room audit asks it too.
	...
	// Counts leaves, never direct members, so a group of groups reports the sensors it actually watches.
	...
	// The registry lists rows: a disabled or never-loaded entity still comes back from EntitiesInArea with no
	// state at all, and unavailable and unknown go too. The cost is that a lamp offline at start-up stays out of
	// its area until the engine is rebuilt, which the Configuration page can do without a restart.
	...
	// Every motion source the area yields, groups already preferred over their own members: a group and its
	// members are the same movement two or three times, each re-arming the vacancy timer.
	...
	// The per-room exclude is applied here so a household can take one sensor out of the mean by id: a fridge's
	// own probe is answered by naming it, and the whole room keeps its average.
