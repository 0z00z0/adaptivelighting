...
/// </summary>
/// <remarks>
///     A <c>null</c> store means the same as an empty one: "we do not know", never "a boundary was crossed".
/// </remarks>
...
/// <remarks>Shaped like <see cref="LastSeen.LastSeenDocument"/>, so the two files read alike.</remarks>
...
/// <remarks>
///     The path is derived from the configuration document's, which is the only directory on a Home Assistant box
///     that survives a redeploy. Writes go to a uniquely named temp file and are moved into place with the
///     previous version kept as <c>.bak</c>. Written on every change, not batched: the write a flush timer would
///     delay is the one a restart is about to need. Nothing here throws; every failure is a warning and an answer
///     of "we do not know".
/// </remarks>
public sealed class LastPeriodStore : ILastPeriodStore
{
	...
	/// <summary>
	///     Creates a store whose file sits beside <paramref name="configFilePath"/>. Only that path's directory
	///     and file name stem are used; the document itself is never touched here.
	/// </summary>
