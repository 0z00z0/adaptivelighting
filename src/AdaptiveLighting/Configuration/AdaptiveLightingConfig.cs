...
/// <summary>
///     Root of the adaptive lighting configuration document. One per NetDaemon host, bound from a YAML file whose
///     top-level key is the fully qualified name of this class.
/// </summary>
public class AdaptiveLightingConfig
{
	...
	///     validation instead of having discovery quietly fill the field in. Period ids are minted here, so a seed
	///     document is already in the shape the migration produces and never takes the migrating write.
	/// </remarks>
...
public class GlobalConfig
{
	...
	/// <summary>Entity gating the whole engine; <c>null</c> means the built-in app switch. Read through <see cref="EffectiveKillSwitchEntity"/>, never directly.</summary>
	...
	/// <summary>How long after a command a state change on the same light is still read as the engine's own.</summary>
	...
	///     light discovery only, never to sensors, and the exclude label always wins over it.
	/// </summary>
	...
	/// <summary>Device classes qualifying a <c>binary_sensor</c> as a motion source. Read <see cref="EffectiveMotionDeviceClasses"/>.</summary>
	/// <remarks>
	///     The default stays empty because the .NET configuration binder appends bound list items to a non-empty
	///     default, so real values here would leave a configured list of three binding to six.
	/// </remarks>
...
/// <summary>
///     Every knob an area can carry. <see cref="AdaptiveLightingConfig.Defaults"/> supplies the baseline and
///     <see cref="AreaConfig"/> merges its nullable twins over it.
/// </summary>
public class AreaSettings
{
	...
	/// <remarks>
	///     A daylight threshold, since the reading is very often an outdoor sensor a room merely follows. A room whose
	///     sensor genuinely measures the room wants a much lower number set per room.
	/// </remarks>
...
/// <remarks>Ordinals pinned, no member renamed or removed, for the reason <see cref="DarknessSource"/> gives.</remarks>
public enum ColorControl
{
	...
	/// <summary>
	///     Command every colour channel at one value, giving neutral white at the target brightness. For a room whose
	///     lights have no colour temperature, where the schedule's kelvin figure cannot reach them.
	/// </summary>
	EqualChannels = 2
}
...
/// <remarks>
///     Ordinals are pinned and no member may be renamed or removed. An unknown key is silence in both binders; an
///     unknown enum value is a <see cref="FormatException"/> at start-up. Which entity supplies the lux reading is a
///     separate question, answered by <see cref="AreaConfig.LuxSensor"/> or <see cref="AreaConfig.FollowOutdoorLux"/>.
/// </remarks>
public enum DarknessSource
{
	...
	/// <remarks>
	///     Pinned at 3: retiring <see cref="Either"/> left this declared last, which would silently renumber it. Enum
	///     members are inlined into consuming assemblies, and <c>Enum.Parse</c> accepts the bare numeral, so
	///     <c>Darkness: 3</c> in a hand-written file has to keep meaning what it means now.
	/// </remarks>
	Always = 3,

	/// <summary>Retired. Behaves as <see cref="Lux"/>, and stays only so a document that names it still parses.</summary>
	/// <remarks>
	///     The editor does not offer it and <see cref="ConfigNormalizer"/> rewrites it on the next save, but no file can
	///     be proved free of the word. Keeps value 2 while declared last, so the live members' ordinals are untouched.
	/// </remarks>
	Either = 2
}
