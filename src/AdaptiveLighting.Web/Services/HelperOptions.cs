...
/// </summary>
/// <remarks>
///     The house mode and the period select both reconcile a helper against stored rows, and both had their own
///     copy of the rule below. The shapes they render differ — a mode option carries a whole editor, a period row
///     carries one dropdown — but the reconciliation is one answer and belongs in one place.
/// </remarks>
public sealed class HelperOptions
{
	...
	private HelperOptions(List<string> live, List<string> trailing, List<string> unmapped, string? active, bool answered)
	{
		...
		// Rendered whether or not the helper answered; called orphans only when it did. A stored row must not
		// vanish off the screen because the socket blinked, and must not be badged as renamed either.
	...
	/// <summary>
	///     Reconciles <paramref name="live"/> against <paramref name="stored"/>.
	/// </summary>
		...
		string? activeValue = null)
	{
		...
		List<string> trailing =
			[.. storedValues.Where(value => !liveValues.Any(offered => Eq(offered, value)))];

		List<string> unmapped =
			[.. liveValues.Where(value => !storedValues.Any(row => Eq(row, value)))];
	...
	public bool IsLive(string? value) =>
		_live.Count == 0 || _live.Any(offered => Eq(offered, value));

	/// <summary>Whether the helper is standing on <paramref name="value"/> right now.</summary>
	public bool IsActive(string? value) =>
		_active is { Length: > 0 } && Eq(_active, value);

	// HouseModeSync's, not a copy: blank-dropping and case-insensitive de-duplication are the same answer, and a
	// second implementation is how two screens come to disagree about whether "Dag " and "dag" are one option.
