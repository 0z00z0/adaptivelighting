...
/// <summary>
///     One area, as a picker offers it. The counts are the resolver's, after discovery and the ghost filter,
///     never the registry's row count, which says nothing about lighting and includes disabled rows.
/// </summary>
...
/// </summary>
/// <remarks>
///     <see cref="Commanded"/> is what is judged, counted and named. <see cref="InTheRoom"/> is only the sibling
...
///     start-up, so a preview is the engine's answer and not a lookalike of it. Scoped, because
///     <see cref="IHaContext"/> is: one per Blazor circuit, with the registry read live, never snapshotted.
/// </remarks>
public sealed class HaCatalog
{
	...
	///     Dropped by <see cref="Invalidate"/> whenever the page re-reads the document.
	/// </remarks>
	...
	/// </summary>
	/// <remarks>
	///     Discovery's own answers, so a ghost the engine excludes is not offered here either.
	/// </remarks>
	...
	/// <summary>
	///     What the area holds besides what discovery settled on, for the fold that overrules discovery.
	/// </summary>
	/// <remarks>
	///     Derived by subtraction, never by re-deciding: group membership, device identity and the label rules
	///     stay in <see cref="AreaEntityResolver"/>, and this removes whatever discovery already claimed.
	/// </remarks>
	...
	/// <summary>
	///     Drops the cached discovery answers, so the next question is put to Home Assistant afresh. Called by the
	///     page whenever it re-reads the document.
	/// </summary>
	...
	public double? LuxIn(ResolvedArea area, GlobalConfig global)
	{
		...
		// Every sensor at zero or less is pitch dark, not a missing reading. The gate's rule, kept so the two
		// agree at the bottom of the range too.
		if (positive.Count == 0)
			return 0;
	...
	/// <summary>
	///     Every <c>light.*</c> entity the registry lists in an area, unfiltered. Not discovery's list, which
	///     prefers groups and so hides a colour channel's parent. Nothing is named or counted from it.
	/// </summary>
	...
	/// <summary>
	///     The registry's own listing of an area, or nothing when it cannot answer. Unfiltered: each caller applies
	///     its own test.
	/// </summary>
