...
/// <summary>A point in the chart's plot area, in the plot's own user units. Y grows downwards, as SVG does.</summary>
public sealed record CurvePoint(double X, double Y);

/// <summary>One setting the chart asks to have changed. The chart writes nothing itself; the page saves.</summary>
...
public static class LuxCurve
{
	...
	/// <summary>The brightest the axis will ever stretch to.</summary>
	...
	/// <summary>Where a reading sits across the axis, from 0 at <see cref="AxisMinLux"/> to 1 at the top.</summary>
	...
	/// <summary>The reading the shaping handle stands on.</summary>
	...
	/// <summary>Whether the shaping handle has any headroom to move the curve in.</summary>
	...
	/// <param name="fraction">Where along the span between the anchors the handle sits.</param>
	/// <param name="position">How far up the headroom the drag put it.</param>
	public static double GammaFor(double fraction, double position, double min, double max)
	{
		...
		// Both ends held off their asymptotes: a drag onto the top or bottom edge otherwise solves to gamma 0,
		// and Math.Pow(0, 0) is 1, which commands the daylight level in the dark.
