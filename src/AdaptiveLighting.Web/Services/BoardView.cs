...
/// <summary>The stretch of time the board draws, and the arithmetic that turns an instant into a position on it.</summary>
/// <remarks>
///     Both ends are snapped to whole hours, so the window steps once an hour instead of sliding: the gridlines
///     can be one repeating background and the axis labels hold still while being read.
/// </remarks>
...
/// <summary>A moment a room saw movement and did not light: the instant, and the gate that turned it down.</summary>
/// <remarks>
...
{
	/// <summary>Whether this room has nothing to say: no past worth drawing, nothing armed, nothing wrong.</summary>
	/// <remarks>
	///     A refusal has to count. It draws no stretch and arms no timer, so a room that turned movement down would
	///     otherwise be indistinguishable from a room nobody walked through.
	/// </remarks>
	public bool IsQuiet =>
...
/// <summary>Everything the board decides, as pure functions. Nothing here touches the configuration document.</summary>
public static class BoardView
{
	...
	/// <summary>One room's past, as blocks clipped to the board's window.</summary>
	/// <remarks>
	///     The engine publishes on transitions, so each report covers the stretch to the next one and the newest
	///     runs to <paramref name="now"/>. Adjacent stretches merge before they become percentages, so the minimum
	///     width applies to the merged whole.
	/// </remarks>
	...
	/// <summary>The moments this room saw movement and did not light, as marks on its lane.</summary>
	/// <remarks>
	///     <see cref="ActivityView.IsDeclinedMotion"/> is the only test for a refusal; a copy here would let the
	///     board show a mark the timeline has no row for.
	/// </remarks>
	public static IReadOnlyList<LaneRefusal> Refusals(IEnumerable<ActivityEntry> entries, BoardWindow window)
	{
			...
			.ThenBy(pair => pair.Entry.Sequence))
		{
			...
			// Not keyed on the gate. The suppressed-off path republishes on every movement, so a sensor re-firing
			// under a hand-set off paints a smear of ticks inside a pixel of each other.
	...
	// The master switch and an empty house turn every room down at once, and that is already said above the lanes.
	// A refusal makes a lane un-quiet, so it must not be one of those two.
	...
	/// <summary>The one dotted mark ahead of the now-line: when this room's armed timer fires, and what it does.</summary>
	/// <remarks>
	///     The verb comes from the state; <c>NextChangeAt</c> carries none. The window reaches four hours back, so
	///     a stale snapshot would otherwise put a confident "auto resumes" to the left of the now-line. Both a
	///     passed deadline and an unarmed state get no mark.
	/// </remarks>
	...
	/// <summary>Whether the room is dark and waiting, and would light on movement but for a block worth naming.</summary>
	/// <remarks>
	///     Only Sleep and EntityOn: the engine refuses in the order kill switch, disabled, away, sleep, entity, so
	///     those two arise only where the room would otherwise have lit. Internal because the activity timeline
	///     asks the same question, and two copies would let the surfaces disagree.
	/// </remarks>
	...
	///     deadline: an override with no expiry configured stands until it is resumed.
	/// </summary>
	...
	///     set against each other: a house-wide event from six rooms is six reports and one row.
	/// </remarks>
