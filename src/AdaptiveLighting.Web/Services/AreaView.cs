...
/// <remarks>
///     The dashboard's cards and the settings list are two views of the same rooms and share this colour mapping,
///     so they cannot paint one house two ways.
/// </remarks>
public static class AreaView
{
	// Must equal RoomSettings.Keys.Count; a test holds the two together. Enabled is not in the list, the header
	// toggle owns it.
	...
	///     engine last said, and a room with no snapshot yet is idle, not unpainted.
	/// </summary>
	...
	/// <summary>The live reports the dashboard gives a card to: those from rooms the owner has switched on.</summary>
	/// <remarks>
	///     The engine keeps observing and publishing switched-off rooms, so this is a question about the document,
	///     not about the report. Matched by area id first and display name second, the same join the snapshot cache
	///     and the settings list use.
	/// </remarks>
	...
	/// <summary>How many rooms the owner has switched off, for the footer under the grid.</summary>
	/// <remarks>
	...
	///     onboarding offered there would hide the real problem.
	/// </param>
	...
	// A report matching no room at all is shown, never hidden: a card dropped because the document changed
	// underneath it is indistinguishable from a fault.
