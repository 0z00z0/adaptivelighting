...
/// <param name="Suspicious">The lights that look like something other than room lighting, each with its reason.</param>
/// <param name="Others">The names of the remaining lights, in the order the resolver settled them.</param>
/// <param name="Advice">
///     How to narrow what the room commands, or <c>null</c> when the house already has an include label.
/// </param>
...
/// <summary>
///     The note a room shows when somebody switches it on: what "on" now reaches, and which of it looks like
///     something other than room lighting.
/// </summary>
/// <remarks>
...
public static class SwitchOnWarning
{
	...
	///     <see cref="GlobalConfig.IncludeLabel"/> as it stands. Set, the household has already said which lights
	///     the app may manage, so the advice is dropped.
	/// </param>
	...
	public static SwitchOnNote? For(string roomName, RoomLights lights, string? includeLabel)
	{
		...
		// ReasonFor, not Review: only Commanded is judged, counted and named, while InTheRoom is the sibling
		// context the colour-channel rule needs. A lamp reached through a group is not in Commanded, and its
	...
	/// <summary>The way to narrow what every room commands, named where it lives.</summary>
