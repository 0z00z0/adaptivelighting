...
/// <param name="Id">
///     The stored id, and the <c>data-theme</c> value where there is one. Stored in the browser and read back by
///     <c>theme.js</c> before the first paint, so renaming one drops every browser that had chosen it. Add themes;
///     do not rename them.
/// </param>
...
public static class AppThemes
{
	...
	///     this build no longer ships. Handed the list on its script tag, so the ids are defined only here.
	/// </remarks>
	public static string DataThemeIds { get; } =
		string.Join(' ', All.Select(theme => theme.DataTheme).OfType<string>());

	/// <summary>
	///     The theme a stored id names, or <see cref="System"/> when it names nothing this build still ships. An
	///     id from a withdrawn theme has to land somewhere real; a missing palette renders dark on a light desk.
	/// </summary>
	...
	public static AppTheme Resolve(string? storedId) =>
		// Ordinal: these are storage keys, not words, and a Turkish locale lower-casing an I only fails elsewhere.
