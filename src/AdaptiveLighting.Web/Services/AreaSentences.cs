...
/// <summary>A room's behaviour, projected into the handful of sentences the overview shows.</summary>
/// <remarks>
...
public static class AreaSentences
{
	...
	/// <summary>The ladder, plus the room's own value when that is not already a rung.</summary>
	...
	// The wording is standalone: the same list fills a bare dropdown in the levels table, with no sentence around
	// it to lean on.
	...
	/// <summary>Why a room with no colour temperature is offered no kelvin, worded in one place.</summary>
	...
	/// <returns>The movement rule, the hand rule, then the flags and the warmth where those apply.</returns>
	...
	// Auto and Kelvin both end with the schedule's kelvin reaching the lights. Only the third answer changes what
	// the levels table can offer, so only it earns a paragraph.
	...
	// while the house also says 10 min has still made a choice. With no room, None, since defaults inherit from
	// nothing.
