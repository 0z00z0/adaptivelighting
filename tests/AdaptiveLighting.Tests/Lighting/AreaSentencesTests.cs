...
/// <summary>The sentences the room page renders, and where each token's value came from.</summary>
[TestClass]
public sealed class AreaSentencesTests
{
	...
	/// <summary>Asserted on the sentence, not on its tokens: "dim to 50 % for 30 s" and "dim to 30 s for 50 %" hold the same tokens.</summary>
	[TestMethod]
	...
	/// <summary>There is no disabled sentinel: off is <see cref="DarknessSource.Sun"/>, so it is keyed on Darkness and leaves LuxThreshold alone.</summary>
	[TestMethod]
	...
	/// <summary>Provenance comes from the schema's null; comparing values would erase a room that pins the same 10 min the house says.</summary>
	[TestMethod]
	...
	/// <summary>A key that drifts from the schema turns <c>case nameof(AreaSettings.VacancyTimeoutSeconds)</c> into a branch nothing reaches.</summary>
	[TestMethod]
