...
/// <summary>What a room's switch says the moment it is turned on; which lights get flagged is <see cref="LightAuditTests"/>.</summary>
[TestClass]
public sealed class SwitchOnWarningTests
{
	...
	// The resolver prefers a group, so a lamp reached through one is absent from the commanded list. The sibling
	// check reads the room's own lights, or a channel's parent is never found.
	[TestMethod]
