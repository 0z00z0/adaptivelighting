...
/// <summary>Setting rooms up from Home Assistant: the first start, and every later "set up rooms again".</summary>
/// <remarks>The counting helpers read the settings off <see cref="AreaConfig"/> by reflection, so the warning can never count short.</remarks>
[TestClass]
public sealed class AreaSetupServiceTests
{
	...
	public void A_Rebuild_Re_Guesses_The_Role_From_The_Area_Name()
	{
		...
		// A hallway whose guessed behaviour was switched back off.
	...
	// The editor and the re-setup warning read the same count, so neither can miss the daylight-brightness settings.
	[TestMethod]
	...
	private static readonly HashSet<string> NotASetting = new(StringComparer.Ordinal)
	{
		...
		// twin, so they count as pinned instead of as overrides.
	...
	/// <remarks>One id per list, so each destroyed field is worth one count. A shape nothing here can fill throws instead of being skipped.</remarks>
