...
/// <summary>
///     The snapshot cache's lifetime.
/// </summary>
/// <remarks>
///     The cache holds one DI scope for the life of the process and NetDaemon's scoped <c>IHaContext</c> is
///     <see cref="IAsyncDisposable"/> only, so disposing that scope synchronously throws. The throw surfaced as
///     "Failed to start host" and killed the process, so the cache must own no synchronous disposal route.
/// </remarks>
[TestClass]
public sealed class AreaSnapshotCacheTests
{
	/// <summary>
	///     An <see cref="IHaContext"/> that can only be disposed asynchronously, as NetDaemon's scoped context is.
	///     Only <see cref="IHaContext.Events"/> is exercised; the rest exists because the interface does.
	/// </summary>
