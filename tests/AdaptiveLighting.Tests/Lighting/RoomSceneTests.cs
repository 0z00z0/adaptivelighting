...
/// <summary>The two per-room scenes: one run in place of switching the room on for movement, one in place of switching it off when the room goes empty.</summary>
// Each replaces one transition and nothing else, so most of what is asserted is what a scene leaves alone: the
// auto-on gates, the leaving sweep, and a hand at the switch.
[TestClass]
public sealed class RoomSceneTests
{
	...
	private static Fixture Build(
		...
		new AreaTestBuilder()
			...
			.Settings(settings =>
			{
				...
				settings.SleepBlocksAutoOn = sleepBlocksAutoOn;
	...
	// The scene's own light changes carry neither a user nor a parent, the detector's definition of a wall switch,
	// so without a declared expectation the room overrides itself the instant it scenes.
	[TestMethod]
