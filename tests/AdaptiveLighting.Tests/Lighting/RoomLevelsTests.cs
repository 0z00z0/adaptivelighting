...
/// <summary>What a room runs instead of the schedule, as the levels card reads and writes it.</summary>
[TestClass]
public sealed class RoomLevelsTests
{
	...
	/// <summary>Both the card and <c>CircadianCalculator.LevelsOf</c> skip empty rows before taking the first match.</summary>
	// Reachable only on a hand-edited file; save normalises empty rows away.
	[TestMethod]
	...
	/// <summary>Period names match case-insensitively, as the engine matches them; ordinal matching would orphan a recased period's row.</summary>
	[TestMethod]
	...
	/// <summary>A padded <c>Period</c> survives load, since nothing trims a reference that already resolves to an id.</summary>
	// The engine trims before matching, so an untrimmed card writes a second row beside the one it failed to find.
	[TestMethod]
	public void A_Padded_Period_Id_In_The_Document_Is_Edited_In_Place()
	{
		...
		Assert.AreEqual("  dag-9x2k  ", room.Levels.Single().PeriodId, "nothing on the read path trims this");

		CircadianCalculator engine = new(
			read.Config.Periods,
			read.Config.Global,
			() => new SunTimes(new TimeOnly(6, 0), new TimeOnly(21, 0)),
			room.Levels);

		Assert.AreEqual(40, engine.GetPeriodTarget("dag-9x2k")!.BrightnessPct, "the engine is already applying this row");

		RoomLevels.SetBrightness(room, "dag-9x2k", 45);

		Assert.AreEqual(1, room.Levels.Count, "the edit belongs on the row the engine is already applying");
		Assert.AreEqual(45, room.Levels.Single().BrightnessPct);

		RoomLevelRow row = RoomLevels.Rows(read.Config.Periods, room).Single(r => r.PeriodId == "dag-9x2k");
		Assert.AreEqual(45, row.BrightnessPct);
		Assert.AreEqual(LevelSource.Room, row.Brightness);
		Assert.AreEqual(0, RoomLevels.Orphans(read.Config.Periods, room).Count, "and it was never stranded");
	}
