...
public sealed class BoardViewTests
{
	...
	// The window reaches four hours back, so a stale snapshot would otherwise draw a prediction to the left of the
	...
	/// <remarks>BoardView.Clock renders through ToLocalTime, so never assert a literal time here; compare against the same projection.</remarks>
	[TestMethod]
	...
	// A refusal makes a lane un-quiet, so drawing a house-wide one per room would empty the quiet shelf and give
