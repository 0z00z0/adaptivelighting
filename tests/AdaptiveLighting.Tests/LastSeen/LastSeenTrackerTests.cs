...
/// <summary>A Home Assistant restart resets every timestamp in the house, and the record must not believe it.</summary>
[TestClass]
public sealed class LastSeenTrackerTests
{
	...
	private sealed class Fixture : IDisposable
	{
		...
		/// <summary>A tracker over the same directory and house, not started.</summary>
		...
		/// <summary>One sensor dead for a week and the rest staggered, a spread a restart does not produce.</summary>
	...
	public void A_Restart_Burst_Does_Not_Advance_The_Record()
	{
		...
		// A restart collapses every timestamp to one instant: 51 motion sensors all reading the same 2.3 hours.
	...
	public void A_Sensor_Whose_Value_Never_Changes_Is_Still_Counted_As_Alive()
	{
		...
		// 3 lx all night: the state never moves and the timestamp does, so counting value changes would call a healthy sensor dead.
	...
	public void A_Sensor_That_Stops_Reporting_Goes_Silent_Even_While_Its_Value_Sits_There()
	{
		...
		// Nothing touches the timestamp, because nothing reported.
	...
	public void A_Fresh_Install_Against_A_Long_Running_Home_Assistant_Seeds_From_Its_Timestamps()
	{
		...
		// Nothing has reset these, so they stay honest evidence.
		Assert.AreEqual(Noon - TimeSpan.FromDays(7), tracker.LastSeenUtc(Dead));
	...
	public void The_Home_Assistant_Timestamp_Survives_The_Wire_Format_Exactly()
	{
		...
		// A timestamp arriving without a UTC marker and read as local time is silently hours out, and every threshold with it.
	...
	public void The_Files_Hold_What_They_Are_Named_For()
	{
		...
		// One file per class, and one per domain for a classless entity.
	...
	public void Nothing_Is_Dropped_By_The_Split()
	{
		...
		// The shapes a real instance has, including a device class that must be sanitised into a file name.
		fixture.Ha.Set("light.kitchen", "on", fixture.Now - TimeSpan.FromMinutes(3));
		fixture.Ha.Set("sensor.washing_machine", "42", fixture.Now - TimeSpan.FromMinutes(3), "power");
	...
	public void One_Changed_Entity_Writes_One_File()
	{
		...
		// Rebuilding every file would multiply the write cost by the number of classes, and there are dozens.
		fixture.Advance(TimeSpan.FromMinutes(1));
	...
	public void A_Pre_Split_Catch_All_Is_Redistributed_Without_Losing_A_Record()
	{
		...
		// Older in Home Assistant than in the cache, so the carried-over value survives the first census.
