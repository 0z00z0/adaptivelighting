...
public sealed class LastSeenBucketsTests
{
	...
	public void A_House_Can_Name_Its_Own_Device_Classes()
	{
		...
		// A house that renamed motion has not renamed the file: an entity still declaring device_class "motion" is
		// filed by its class like anything else, and the motion file keeps holding only what the house calls motion.
	...
	{
		// binary_sensor.device_class light is a real Home Assistant class: it detects light, it is not a lamp.
		// Filing it under "light" would change what the light file holds, which is the one thing the split may not do.
	...
	{
		// Most file systems this runs on are case-insensitive, so two keys differing only in case would be two
		// divergent histories fighting over one file.
	...
	{
		// device_class is data from an external system that reaches the file system. Anything a custom integration
		// can put in a string has to come out of here as a file name, and one that cannot escape its directory.
	...
	{
		// Japanese for "temperature" and "humidity": a plausible class from a locale-aware custom integration, and
		// nothing in it survives an ASCII allow-list.
	...
	{
		// The fingerprint ends up in a file name, so it cannot come from a per-process hash seed: a record written
		// today would be looked for under a different name tomorrow.
