...
- **The daylight curve as a per-period mode is built and awaiting review.** Each period chooses between
  *specify brightness*, the default and unchanged, and *use daylight curve*, where the curve decides. The chart
  shows only when a period claims it; a claiming period hides its percentage but keeps the stored value. The
  curve is free across 0-100 % and replaces the level rather than adding to it. The reading comes from the
  house outdoor sensor, overridable per room, and a configuration with nothing to read is warned about.
  `LuxBrightnessEnabled` is retired. The curve's dark end is seeded from the period that claims it, at half its
  brightness, on the first claim only.
  - Two things are wanted before it merges: **somebody has to look at it**, since no screenshot was possible and
    only the rendered DOM was measured; and a decision on **the boundary into or out of a curve period being a
    step rather than a blend**, which is fixable only by moving the curve inside the calculator and breaking the
    documented period-then-curve-then-clamp order.
  - A house that had the old per-room lift on loses it until a period claims the curve. Nothing is migrated, by
    decision; the curve's shaping settings survive untouched.
