...
This page describes what the system does. For installing and operating it, see
...
only when the room is dark. They dim as a warning before switching off. Touch a switch and the
automation leaves your setting alone for a while. When the house empties the lights sweep off; the
first person home is met by the entry lights. One master switch pauses everything, and every room
keeps reporting what it would have done.
...
- **A room** is one Home Assistant area, with its lights, its motion sensors and any light-level
  ...
  reported, but never commanded.
- **The schedule** is one table for the whole house, cut into periods — morning, day, evening,
  night. Each period sets a brightness and a colour temperature. Boundaries are clock times or sun
  events, and levels blend across a boundary rather than stepping. A period can wait for movement
  instead of starting on the clock.
- **The house** holds what every room shares: who is home, which house mode is active, and whether
  the master switch is on.
- **Who made a change** is worked out for every change to a light: the engine's own, or a person's.
  That is what makes manual changes stick.
...
A room reads a light-level sensor or the sun's height — the **How the room decides it's dark**
setting. The default is *Sensor*: the room's own reading decides, and the sun is never consulted.
*Sun* reads sun elevation instead.

- **A room with nothing to read counts as dark** wherever a sensor is consulted, and movement lights
  it. That covers a room with no light-level sensor and a room whose sensors have all stopped
  reporting. Sensors that go quiet are named in the log.
...
- **A house-wide outdoor sensor is opt-in per room.** Name one under *Finding lights & sensors*,
  then give `FollowOutdoorLux: true` to the rooms that should read it. This is set in the
  configuration file; the UI has no control for it.
- **The default threshold is 1000 lx**, because the reading is usually an outdoor one. A sensor that
  really measures the room wants a much lower number.
...
Separately from the gate above, a room can lift its lights as it gets brighter outside. It is off
until you switch it on, and it only ever adds light: *Brightest it goes* is the ceiling, and a
period already brighter than that is left alone.
...
- **A Home Assistant light group wins over its members.** The group is commanded; the entities
  inside it are not commanded separately.
- **Several entities on one Home Assistant device count as one fixture**, so an RGBW lamp's combined
  ...
  watches different places.
- **Three Home Assistant labels steer it.** Anything carrying the *never touch* label is invisible.
  Anything carrying the *counts as motion* label is treated as a motion sensor whatever its type.
  Name an *only manage lights with* label and only lights carrying it are driven; leave it empty and
  every light found is managed.
...
The engine records what it expects before it commands a light, and compares what comes back. A
change that does not match is a person's, and the room hands over:
...
- **A room that cannot be resolved is skipped and the rest keep running.** The message names the
  real ids.
- **A problem with the document itself** stops the engine commanding anything and posts a Home
  ...
  you fix it.
- **Settings survive a redeploy.** The live configuration file lives outside the deploy folder, and
