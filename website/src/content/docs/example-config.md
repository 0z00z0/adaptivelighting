...
`LuxSensorStaleAfterMinutes` on the house.
...
AdaptiveLighting.Configuration.AdaptiveLightingConfig:
  ...
  Global:
    ...
    # flag: state "off" pauses the engine. Set it false for a true kill switch,
    # where "on" pauses it.
    ...
    #   ExcludeLabel — never let the app touch it. Always wins.
  ...
  Defaults:
    ...
    #            stopped reporting, counts as dark and lights on movement. The
    #            sun is never consulted here.
...
house on any movement and fight the real rooms. Leave it out. Set-up will not propose it unless it
has a motion sensor of its own; if it does, switch it off and leave it off.

**A room needs both halves.** An area with motion but no lights has nothing to offer, and one with
lights but no way to sense people cannot do motion-driven lighting. Set-up skips both.

**Explicit lists are for when the registry is wrong.** Everything else is better left to discovery,
which stays true across a rename. Fixing the area assignment in Home Assistant is usually better
than either.

## What refuses to save

Some problems stop the engine commanding anything and are listed on the Configuration page: no
periods, a `Start` that cannot be parsed, two periods with the same name or the same start,
duplicate room names, negative timeouts, a warning dim longer than the time the lights stay on, a
value out of range.
